...
<div class="item-actions">
    <div class="edit" data-id="${response.data[i].id}" id="editItem">edit</div>
      ...
      const editProductNew = () => {
        ...
        for (let i = 0; i < allEditItems.length; i++) {
          allEditItems[i].addEventListener("click", function () {
            let editingItemId = parseInt(
              allEditItems[i].getAttribute("data-id")
            );
            ...
            editItem.onclick = () => {
              ...
              let editItemObj = {
                ...
                current_price: parseFloat(
                  document.getElementById("editItemCurrPrice").value
                ),
                previous_price: parseFloat(
                  document.getElementById("editItemPrevPrice").value
                ),
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          let addProductData = {
            ...
            price: {
              usd: parseFloat(
                document.getElementById("addItemCurrPriceUsd").value
              ),
              eur: parseFloat(
                document.getElementById("addItemCurrPriceEur").value
              ),
              uah: parseFloat(
                document.getElementById("addItemCurrPriceUah").value
              ),
            },
...
if (
  window.location.pathname === "/admin-panel-new.html" &&
  getCookie("auth_token") !== null
) {
  adminPanel();
}
